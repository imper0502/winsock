...
int main(int argc, char *argv[]) {
  ...
  nResult = WSAStartup(0x101, (LPWSADATA)&wsadata);
  if(nResult!=0) {
  ...
  servAddr.sin_port        = htons(5554); // 指定IPPORT_ECHO 為 echo port
...
void *ThreadMain(void *threadArgs) {
  SOCKET clntSock = ((struct ThreadArgs *)threadArgs)->_clntSock;
  free(threadArgs);
  char buf[MAXLINE];
  int n;
  ...
  while(n>0) {
    ...
    sleep(1);
    //<<<<<<<<<<<<<<<<DO END <<
    if(send(clntSock, "#", strlen("#"), 0) != strlen("#")) {
      printf("send faild.\n");
    }
    n = recv(clntSock, buf, MAXLINE, 0);
    buf[n] = '\0';
